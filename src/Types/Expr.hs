...
cofreeTraverse
    ...
    => (a -> t (Cofree t a) -> f b) -> Cofree t a -> f (Cofree t b)
cofreeTraverse f (a :< m) =
    (:<) <$> f a m <*> sequenceA (fmap (cofreeTraverse f) m)

cofreeCata
    :: (Applicative f)
    => (a -> f b -> b) -> Cofree f a -> b
cofreeCata f (a :< m) = f a (cofreeCata f <$> m)
...
instance Show1 BareExpr where
    showsPrec1 n expr =
        case expr of
            ...
            l :@: r ->
                showParen (n > 10) $
                showString "`" . showsPrec n l . showString " " . showsPrec 10 r
            BinOp s e e' ->
                showParen (n > 1) $
                showsPrec 2 e . showString (" " ++ s ++ " ") . showsPrec 2 e'
            If p t e ->
                showString "if " .
                showsPrec n p .
                showString " then " .
                showsPrec n t . showString " else " . showsPrec n e
            Lam v e ->
                showString "^ " .
                showString v . showString " -> " . showsPrec n e
